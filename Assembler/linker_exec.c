 ...
 * linker_exec.c
 *
 * Minimal linker for the SMPL assembler/loader pipeline.
 * Responsibilities:
 * - Read multiple relocatable object modules (`.o`) and their table files (`.t`)
 * - Build a global external symbol table (ESTAB) from H/D records
 * - Assign base addresses to each module and resolve all inter-module references
 * - Produce a linked executable `.exe` file and a combined DAT `.t` file
 *   for use by the relocating loader.
 *
 * Usage:
...
static void parse_t_file(Module *mod) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (section == SEC_SYMTAB) {
            // Expected format: "Symbol: LOOP       Address: 0"
            char sym[32];
            int addr = 0;
...
static void link_modules(const char *out_prefix, Module *mods, int mod_count, const SymMap *estab) {
    ...
    if (!exe) {
        fprintf(stderr, "ERROR: could not open .exe for writing: %s\n", exe_path);
        return;
    }
    ...
    for (int mi = 0; mi < mod_count; mi++) {
        ...
        FILE *fp = fopen(mod->o_path, "r");
        if (!fp) {
            fprintf(stderr, "ERROR: could not open .o file: %s\n", mod->o_path);
            continue;
        }
    ...
    if (!tfp) {
        fprintf(stderr, "ERROR: could not open .t for writing: %s\n", t_path);
        return;
    }
...
int main(int argc, char **argv) {
    if (argc < 4 || ((argc - 2) % 2) != 0) {
        fprintf(stderr, "Usage: %s <out_prefix> <mod1.o> <mod1.t> [<mod2.o> <mod2.t> ...]\n", argv[0]);
        return 1;
    }
