 ...
 * loader.c
 *
 * Implementation of the relocating loader for the SMPL toolchain.
 * Responsibilities:
 * - Read the linked executable `.exe` file and map LC values to memory addresses
 *   using a user-specified load point.
 * - Interpret opcodes and operand bytes so that the memory array `M[]` stores
 *   instructions in the same format as the assembler/linker expect.
 * - Read the DAT (Direct Address Table) from the `.t` file and apply relocation
 *   by adjusting all absolute 16-bit addresses with the chosen load point.
 * - Provide utilities to print the decoded memory contents.
 */
...
// Mapping from hex opcode to instruction length in bytes in memory.
...
// Return the instruction size in bytes for a given hex opcode; 1 byte for data.
...
// Check whether the given hex value corresponds to a known SMPL opcode.
...
// Convert a decimal address into two 2-digit strings, e.g., 1033 -> "10" "33".
void intToTwoCharString(int value, char *high, char *low) {
    sprintf(high, "%02d", value / 100);
    sprintf(low, "%02d", value % 100);
}

// Insert or update a byte at a specific memory address in the global array M[].
...
// Look up a byte in M[] by its address; returns NULL if not present.
...
// Parse the `.exe` file and load all bytes into M[] shifted by `loadpoint`.
static void loadExeFile(const char *exe_file, int loadpoint) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        // If the first token is an instruction opcode, store it and its operands.
        ...
        // Otherwise treat the first token as a data byte.
        else {
            char str[8];
...
// For each address in DAT, add the loadpoint to the 16-bit value stored there.
...
// High-level loader routine: ask for load point, load `.exe`, apply relocation, print memory.
void runLoader(const char *exe_file, const char *t_file) {
    memory_count = 0;
    memset(M, 0, sizeof(M));
    
    unsigned int loadpoint;
    printf("Enter load point (decimal or hex, e.g., 1000 or 0x3E8): ");
